...
exports.handler = async function (event) {
  try {
    ...
    if (!action || !table) return { statusCode: 400, body: JSON.stringify({ error: 'Missing action or table' }) };
    ...
    if (action === 'select') {
      ...
      return { statusCode: 200, body: JSON.stringify({ data: resData }) };
    }

    if (action === 'insert') {
      if (!data) return { statusCode: 400, body: JSON.stringify({ error: 'Missing data to insert' }) };
    ...
    if (action === 'update') {
      if (!data || !filters) return { statusCode: 400, body: JSON.stringify({ error: 'Missing data or filters for update' }) };
    ...
    if (action === 'delete') {
      if (!filters) return { statusCode: 400, body: JSON.stringify({ error: 'Missing filters for delete' }) };
      Object.keys(filters).forEach((k) => { query = query.eq(k, filters[k]); });
